...
- 装饰与所属 Ground Style 的边缘至少保持一格距离：任一四邻格不属于同一
  Style 的格不能被装饰占用，留给基础地皮的过渡 RuleTile。
...
5. 对每条装饰：按 `R` 和 Style 面积计算种子数，只在离所属 Style 边缘至少
   一格的内部区域紧凑蔓延到最大次数，保留成功占格，再将 RuleTile 写到
   装饰 Tilemap。
