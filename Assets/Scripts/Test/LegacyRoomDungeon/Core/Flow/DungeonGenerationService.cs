...
namespace CrystalMagic.Core
{
    internal static class DungeonGenerationService
    {
        ...
        public static IEnumerator GenerateForTransition(LoadGameContext context, string targetSceneName)
        {
            ...
            if (theme != null && theme.UseOpenFieldGeneration)
            {
            ...
            string targetSceneName)
        {
            ...
            terrainConfig.Width = Mathf.Max(8, generationConfig?.DimX ?? terrainConfig.Width);
            terrainConfig.Height = Mathf.Max(8, generationConfig?.DimY ?? terrainConfig.Height);
            ...
            while (true)
            {
                ...
                if (!HasValidExitGuard(sceneData, layout.ExitInterestPoint, isBossFloor))
                {
                    ...
                    PublishProgress(
                        ...
                        $"Attempt {attemptIndex} cannot deploy the exit guard squad; continuing with the next seed.");
                    yield return null;
                    continue;
                }
