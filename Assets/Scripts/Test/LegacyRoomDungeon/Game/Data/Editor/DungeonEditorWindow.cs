...
namespace CrystalMagic.Editor.Data
{
    public partial class DungeonEditorWindow : EditorWindow
    {
        ...
        private void DrawTabs()
        {
            ...
            if (GUILayout.Toggle(_selectedTab == DungeonTab.OpenField, "Open Field", EditorStyles.toolbarButton))
                SelectTab(DungeonTab.OpenField);
            GUILayout.FlexibleSpace();
            EditorGUILayout.EndHorizontal();
        }
        ...
        private void DrawThemeOverview(DungeonThemeData row)
        {
            ...
            GUILayout.Space(8f);
            EditorGUILayout.HelpBox("Configure terrain tiles, landmarks, squads, treasure items, and boss squads in the Open Field tab.", MessageType.None);
        ...
        private void DuplicateTheme()
        {
            ...
            copy.Mob2PoolId = -1;
            copy.Mob3PoolId = -1;
            copy.Treasure1PoolId = -1;
            copy.Treasure2PoolId = -1;
            copy.Treasure3PoolId = -1;
            copy.BossRoomIds = new List<int>();
            copy.EnsureValid();
            _themes.Add(copy);
            _selectedThemeIndex = _themes.Count - 1;
            _selectedThemeSubTab = ThemeSubTab.Overview;
        ...
        private string GetListTitle()
        {
            return _selectedTab switch
            {
                ...
                DungeonTab.OpenField => $"Themes / Open Field ({_themes.Count})",
                _ => string.Empty,
            };
        }
