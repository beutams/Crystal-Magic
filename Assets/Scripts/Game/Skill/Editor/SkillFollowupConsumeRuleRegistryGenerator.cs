...
{
    public static class SkillFollowupConsumeRuleRegistryGenerator
    {
        private const string OutputPath = "Assets/Scripts/Game/Skill/SkillFollowupConsumeRuleRegistry.cs";

        [MenuItem("Tools/Registry/Skill Followup Consume Rule")]
        public static void Generate()
        {
            List<FactoryRegistryGeneratorUtility.MappedType> dataTypes =
                FactoryRegistryGeneratorUtility.CollectMappedTypes(typeof(CrystalMagic.Game.Data.SkillFollowupConsumeRuleData), subclassOnly: true);
            List<FactoryRegistryGeneratorUtility.MappedType> runtimeTypes =
                FactoryRegistryGeneratorUtility.CollectMappedTypes(typeof(CrystalMagic.Game.Skill.SkillFollowupConsumeRule), subclassOnly: true);

            string content = BuildRegistry(dataTypes, runtimeTypes);
            RegistryGeneratorUtility.WriteFile(OutputPath, content);
            AssetDatabase.Refresh();

            Debug.Log($"[SkillFollowupConsumeRuleRegistryGenerator] Generated {OutputPath}.");
        }
            ...
            List<FactoryRegistryGeneratorUtility.MappedType> runtimeTypes)
        {
            ...
            sb.AppendLine("// Use menu: Tools/Registry/Skill Followup Consume Rule");
            sb.AppendLine();
            ...
            sb.AppendLine("{");
            sb.AppendLine("    public static class SkillFollowupConsumeRuleRegistry");
            sb.AppendLine("    {");
            ...
            List<FactoryRegistryGeneratorUtility.MappedType> runtimeTypes)
        {
            ...
            sb.AppendLine("        }");
            sb.AppendLine();
            sb.AppendLine("        public static string GetRuleKey(SkillFollowupConsumeRuleData ruleData)");
            sb.AppendLine("        {");
            ...
            sb.AppendLine("        }");
            sb.AppendLine();
            sb.AppendLine("        public static SkillFollowupConsumeRuleData CreateRuleData(string key)");
            sb.AppendLine("        {");
