...
namespace CrystalMagic.Game.Unit
{
    ...
    public class EntitySpawnRegistryAuthoring : MonoBehaviour
    {
        ...
        [SerializeField, HideInInspector] private List<GameObject> _vfxPrefabs = new();
        ...
        private void SyncPrefabLists()
        {
            ...
            SyncPrefabsFromFolder(VfxFolder, _vfxPrefabs);
        }
        ...
        private sealed class Baker : Baker<EntitySpawnRegistryAuthoring>
        {
            public override void Bake(EntitySpawnRegistryAuthoring authoring)
            {
                ...
                DynamicBuffer<VfxEntityPrefabRegistryEntry> vfxBuffer = AddBuffer<VfxEntityPrefabRegistryEntry>(entity);
                ...
                AddVfxEntries(GetBakePrefabs(authoring.VfxPrefabs, VfxFolder), vfxBuffer);
            }
            ...
            private void AddVfxEntries(IReadOnlyList<GameObject> prefabs, DynamicBuffer<VfxEntityPrefabRegistryEntry> buffer)
            {
                HashSet<string> usedNames = new(StringComparer.Ordinal);
                for (int i = 0; i < prefabs.Count; i++)
                {
                    ...
                    buffer.Add(new VfxEntityPrefabRegistryEntry
                    {
                        Name = new FixedString128Bytes(prefab.name),
                        Prefab = GetEntity(prefab, TransformUsageFlags.Dynamic),
                    });
