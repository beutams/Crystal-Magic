...
namespace CrystalMagic.Game.Data
{
    ...
    public static class BehaviorNodeDataRegistry
    {
        ...
        public static IReadOnlyList<FactoryTypeInfo> TypeInfos => BehaviorTreeRegistry.BehaviorNodeDataTypeInfos;

        public static IReadOnlyList<string> TypeOrder => BehaviorTreeRegistry.BehaviorNodeDataTypeOrder;

        public static bool TryGetNodeType(string typeName, out Type nodeType)
        {
            return BehaviorTreeRegistry.TryGetBehaviorNodeDataType(typeName, out nodeType);
        }

        public static string GetDisplayName(string typeName)
        {
            return BehaviorTreeRegistry.GetBehaviorNodeDataDisplayName(typeName);
        }
        ...
        public static string ResolveTypeName(BehaviorNodeData node)
        {
            ...
            if (!string.IsNullOrWhiteSpace(node.Type) &&
                BehaviorTreeRegistry.ContainsBehaviorNodeDataKey(node.Type))
            {
                return node.Type;
            }

            if (BehaviorTreeRegistry.TryGetBehaviorNodeDataKey(node.GetType(), out string typeName))
                return typeName;
        ...
        {
            if (!BehaviorTreeRegistry.ContainsBehaviorNodeDataKey(typeName))
