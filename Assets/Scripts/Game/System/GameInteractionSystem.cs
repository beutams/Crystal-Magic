...
public partial class GameInteractionSystem : SystemBase
{
    ...
    private void TryExecuteDrop(in GameInteractionRequest request)
    {
        ...
        if (!GameRuntimeStateUtility.TryGetPlayerCharacterData(EntityManager, request.Actor, out CharacterData playerData))
            return;

        BackpackData backpack = playerData.Backpack;
        CharacterPropData props = playerData.Props;
