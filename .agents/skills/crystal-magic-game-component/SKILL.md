...
Use this skill only in the `Crystal-Magic` and `Crystal-Magic-Web` projects. Do not apply these project-specific rules to other projects.
...
- When calling GameComponent-based components, do not add null checks; call them directly through `ComponentType.Instance.Member`.
...
- Put one Authoring, its nested Baker, and its corresponding Component in the same `.cs` file. When that unit Component exposes data through the source-access system, put its one matching `IUnitComponentSource` implementation in that file too.
...
- Keep at most one matching `IUnitComponentSource` per Authoring file.
...
- When adding a Component/Authoring with manually configured prefab data, also add a separate `UnitEditor` attribute drawer under `Assets/Scripts/Game/Data/Editor/` so `UnitEditor` adapts automatically through drawers instead of hardcoding more component branches in `UnitEditorWindow`.
...
- New ECS Authoring files contain exactly one Authoring, its Baker, one corresponding Component, and, when needed for unit source access, its one matching `IUnitComponentSource`.
...
- Prefer minimal line edits when modifying existing files; avoid deleting and recreating whole files unless it is truly necessary.
